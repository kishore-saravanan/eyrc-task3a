...
def main():
    ...
    while flag:
        try: 
            tf_buffer = tf2_ros.buffer.Buffer()
            transform = tf_buffer.lookup_transform('cam_1', 'tool0', rclpy.time.Time(), timeout=rclpy.duration.Duration(seconds=1.0))
            #transform = TransformStamped()

            #transform.header.stamp = node.get_clock().now().to_msg()
            #transform.header.frame_id = 'base_link'
            #transform.child_frame_id = child_frame
            node.get_logger().info("Created lookup the transform from base_link to tool0")
            flag = False


        except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
            node.get_logger().error("Failed to lookup the transform from base_link to tool0")
            continue
